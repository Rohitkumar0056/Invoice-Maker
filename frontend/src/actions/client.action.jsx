...
export const getClientsByUser = (searchQuery) => async (dispatch, getState) => {
    try {
        ...
        const {data} = await axios.get(`${API_BASE_URL}/api/clients/user?search=${searchQuery.search}`, config);

        dispatch ({ type: CLIENT_LIST_SUCCESS, payload: data });
    }
    catch(error) {
        const message = error.response && error.response.data.message ? error.response.data.message : error.message;
        dispatch ({ type: CLIENT_FAIL, payload: message });
        toast.error(message, {
...
export const createClient = (client) => async (dispatch, getState) => {
    try {
        ...
        const {data} = await axios.post(`${API_BASE_URL}/api/clients`, client, config);
...
export const updateClient = (id, client) => async (dispatch, getState) => {
    try {
        ...
        const {data} = await axios.put(`${API_BASE_URL}/api/clients/${id}`, client, config);
...
export const deleteClient = (id) => async (dispatch, getState) => {
    try {
        ...
        await axios.delete(`${API_BASE_URL}/api/clients/${id}`, config);
