...
import { getClientsByUser } from "../actions/client.action";
import { getInvoice, updateInvoice } from "../actions/invoice.action";
...
const EditInvoice = () => {
    ...
    const getTotalCount = async () => {
        try {
            ...
            const response = await axios.get(`http://localhost:4000/api/invoices/count?search=${userInfo?._id}`, config);
    ...
    return (
        <>
            {loading && <div className='bg-slate-100 m-4 p-2 shadow-2xl shadow-green-900 rounded-lg '>
            ...
            {!loading && invoice && <div className="bg-slate-100 m-4 shadow-2xl rounded-lg p-4 flex gap-4 flex-col">
                <div className="mb-4 p-2 flex flex-col md:flex-row md:items-center gap-4">
                    ...
                    <input className="bg-[#6DA5C0] shadow-lg w-[25%] text-white placeholder:text-white font-semibold p-2 rounded-lg" placeholder="Ex: 001" value={invoiceData.invoiceNumber} onChange={e => setInvoiceData({...invoiceData, invoiceNumber: e.target.value})} type="number"></input>
                </div>
                    ...
                    <div>
                        ...
                        {client ?
                          ...
                          : <div className="relative flex flex-col md:flex-row gap-4">
                                ...
                                <button onClick={() => setIsOpen((prev) => !prev)} className="flex items-center overflow-clip justify-between tracking-wider border-4 border-transparent active:border-black duration-100 active:text-black w-fit bg-[#0F959C] shadow-lg p-2 rounded-lg font-semibold text-white">Existing Customer

                                {isOpen ? (<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12.2048 7.29258L18.1189 15.7412C18.49 16.2715 18.1107 17 17.4635 17L6.53652 17C5.88931 17 5.50998 16.2715 5.88114 15.7412L11.7952 7.29258C11.8947 7.1504 12.1053 7.1504 12.2048 7.29258Z" fill="#fff"/></svg>) 
                                : (<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M11.1808 15.8297L6.54199 9.20285C5.89247 8.27496 6.55629 7 7.68892 7L16.3111 7C17.4437 7 18.1075 8.27496 17.458 9.20285L12.8192 15.8297C12.4211 16.3984 11.5789 16.3984 11.1808 15.8297Z" fill="#fff"/></svg>)}

                                {isOpen && loading && <div className='bg-[#0F959C] px-2 absolute top-28 left-0 md:left-[158px] md:top-14 w-48 rounded-lg'>
                                    <Skeleton className=' rounded-lg my-1 focus:outline-none border-2 border-dashed' count={5} /></div>}

                                {isOpen && clients && <div className="bg-[#0F959C] px-2 absolute top-28 left-0 md:left-[158px] md:top-14 w-48 max-h-52 overflow-y-auto flex flex-col items-center rounded-lg">
                                    { clients.map((c) => (
                                        <div onClick={() => { setIsOpen(false); setClient(c);}} className="w-full hover:bg-yellow-400 cursor-pointer rounded-r-lg border-l-transparent hover:border-l-black hover:text-black border-l-4 p-4" key={c._id}>
                            ...
                            {invoiceData.items.map((item, index) => (
                                <tr>
                                    ...
                                    <td className='p-3 font-semibold'><button onClick={() => handleRemoveField(index)} className='bg-red-400 rounded-full p-1'><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <path d="M9 5C9 3.89543 9.89543 3 11 3H13C14.1046 3 15 3.89543 15 5V7H9V5Z" stroke="#000000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                ...
                <div className="flex flex-col mt-4 gap-4 p-4">
                    <div className="flex flex-col md:flex-row gap-4">
                        <div>
                            ...
                            <input className='bg-[#6DA5C0] text-white placeholder:text-white shadow-lg w-[50%] font-semibold p-2 rounded-lg' type="number" value={rates} onChange={e => handleRates(e)} placeholder='0'/>
                        </div>
                        <div>
                            <h2 className='text-gray-700 font-bold mb-3'>Due Date</h2>
                            <input className='bg-[#6DA5C0] text-white placeholder:text-white w-[50%] md:w-[100%] shadow-lg font-semibold p-2 rounded-lg' type="date" value={selectedDate.slice(0, 10)} onChange={(e)=>{ setSelectedDate(e.target.value) }}/>
                        </div>
                    </div>
                    <div>
                        <h2 className='text-gray-700 font-bold mb-3'>Currency</h2>
                        <input className='bg-[#6DA5C0] text-white placeholder:text-white uppercase shadow-lg w-[50%] md:w-[12%] font-semibold p-2 rounded-lg' type="text" value={currency} onChange={(e)=>{ setCurrency(e.target.value) }} placeholder='Ex: INR' />
                    </div>
                </div>
                ...
                <div className="bg-zinc-200 rounded-lg flex flex-col p-4 mt-4">
                    ...
                    <textarea className='bg-[#6DA5C0] text-white placeholder:text-white font-semibold rounded-lg p-2' placeholder="Provide additional details or terms of service" onChange={(e) => setNotes(e.target.value)}  value={notes} />
                </div>
                <div className="flex">
                    <button className='bg-indigo-500 hover:bg-indigo-600 font-bold mt-4 focus:outline-none text-sm text-white py-2.5 px-5 text-center rounded-lg w-fit mx-auto' onClick={handleSubmit}>SAVE AND CONTINUE</button>
                </div>
            </div>
            }
        </>
