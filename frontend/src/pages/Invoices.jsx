...
import { listInvoices, deleteInvoice } from "../actions/invoice.action";
...
const Invoices = () => {
    ...
    return (
        <>
            ...
            {invoices && <div className='h-screen'>
                <div className='bg-slate-100 m-4 shadow-2xl shadow-green-900 rounded-lg p-4 flex gap-4 flex-col'>
                                ...
                                {(invoices.slice((currentPage-1)*5, currentPage*5)).map((invoice) => (
                                    <tr className="text-green-500 font-mono hover:bg-yellow-200 cursor-pointer [&:nth-child(even)]:bg-gray-100 [&:nth-child(even)]:hover:bg-yellow-200 [&:nth-child(odd)]:text-gray-500 transform transition-all hover:scale-[0.99]" key={invoice._id}>
                                        ...
                                        <td className=' p-3 font-semibold text-center'><button onClick={() => handleEdit(invoice._id)} className='bg-gray-100 rounded-full p-2'><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <path d="M5 20H19" stroke="#000000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></button>
                                        </td>
                    ...
                    <div className="bg-gray-200 mt-2 p-2 flex-wrap">
                        <ul className="flex flex-col sm:flex-row justify-center items-center gap-4">
                            <li className="pagination">
                                <button className="bg-white hover:bg-blue-200 font-bold  focus:outline-none text-sm text-blue-500 py-2.5 px-5 text-center rounded-lg w-fit mx-auto" onClick={prePage}>Prev</button>
                            </li>
                            <li>
                                <div className="bg-white font-bold focus:outline-none text-sm text-blue-500 py-2.5 px-5 text-center rounded-lg w-fit mx-auto">{currentPage}</div>
                            </li>
                            <li>
                                <button className="bg-white hover:bg-blue-200 font-bold  focus:outline-none text-sm text-blue-500 py-2.5 px-5 text-center rounded-lg w-fit mx-auto" onClick={nextPage}>Next</button>
                            </li>
                        </ul>
                    </div>
                </div>
