...
const AddClient = ({isVisible, onClose, currentId, setCurrentId}) => {
    ...
    return (
        <>
            <div className="fixed inset-0 z-50 bg-black bg-opacity-25 backdrop-blur-sm flex justify-center items-center" id="wrapper" onMouseDown={handleClose}>
                <div className="bg-white rounded-lg flex flex-col gap-4 p-4 px-6">
                    ...
                    <button className="bg-indigo-500 hover:bg-indigo-600 font-bold focus:outline-none text-sm text-white py-2.5 px-5 text-center rounded-lg w-fit mx-auto" onClick={handleSubmit}>Save Customer</button>
                </div>
            </div>
        </>
    );  
