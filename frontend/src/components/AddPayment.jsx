...
const AddPayment = ({isVisible, onClose, invoice}) => {
    ...
    return (
        <>
            <div className="fixed inset-0 z-50 bg-black bg-opacity-25 backdrop-blue-sm flex justify-center items-center" id="wrapper" onMouseDown={handleClose}>
                <div className="bg-white rounded-lg flex flex-col gap-4 p-4 px-6">
                    ...
                    <input className="bg-gray-100 rounded-lg p-2 px-4 w-80 border-2 border-gray-300" value={payment.amountPaid} onChange={(e) => setPayment({...payment, amountPaid: e.target.value})} type="number" />
                    ...
                    <button className='bg-indigo-500 hover:bg-indigo-600 font-bold  focus:outline-none text-sm text-white py-2.5 px-5 text-center rounded-lg w-fit mx-auto' onClick={handleSubmitPayment}>
                        Save Record
                    </button>   
                </div>
            </div>
